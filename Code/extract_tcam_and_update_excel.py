"""
Extract The Club at Millenia (TCAM) invoices and update comprehensive Excel file.
...
GENERIC_INVOICE_PATTERNS = [f"invoice ({i}).pdf" for i in range(1, 11)]
...
5. Property Name: Verify this is for "The Club at Millenia"
...
VALIDATION FLAGS:
- If property name doesn't match "{PROPERTY_NAME}", flag it
...
def extract_all_tcam_invoices():
    ...
    for i, pdf_path in enumerate(invoice_files, 1):
        ...
        if invoice_data:
            ...
            if invoice_data.get('validation_flags'):
                for flag in invoice_data['validation_flags']:
                    print(f"    🔴 {flag}")
        else:
